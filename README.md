...
5. **배포는 자동이다.** 허브에 push 하면 Vercel 이 자동 배포한다 (반영까지 1~2분).
...
| 러닝 카드 | **본 카드는 메인과 동일 레이아웃, 내용은 신규품번만** (기획/입고/누계/판매율·컬러 표 전부 신규품번 CO 값). **클릭하면 현재고 기준 팝업**: 기초재고(시점재고 07/31) · 추가 기획량 · 입고량(신규) · 현재고(기초+입고) · 판매율 = (구품번 8/1 이후 판매 + 신규품번 누계) ÷ 현재고. 팝업 컬러 표의 기획 = **컬러별 기초재고 + 컬러별 신규입고** (시점재고 시트가 컬러별 행이라 가능), 판매율 = 8/1 이후 누계 ÷ 그 값. 8/1 이후 = 8/1 이 포함된 주(W5, 07/27~)부터 |
| 스타일수·SKU·기획량 | **MDP 파일**(`data/26FW_MDP.xlsb`) MDP 시트를 스타일 블록(AI열 STY=1 시작) 단위로 읽어 **ED열(컨펌여부)='확정' 인 스타일만** 집계 (SKU = 컬러명 AM 행 수, 기획량 = 컬러별수량 AN 합, INNER→TOP · SHOES→ACC). 하위 카테고리는 확정 스타일을 아이템코드(H열)별로 집계해 채운다. MDP 갱신 시 새 파일을 `data/26FW_MDP.xlsb` 로 복사 |
| 전년 비교 | 시즌 구분 없이 신상품 토탈(가을+겨울 합산) 대 전년 동시점, 복종별 블록 |
