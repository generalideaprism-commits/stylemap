...
| 스타일수·SKU·기획량 | **MDP 파일**(`data/26FW_MDP.xlsb`) 집계표 시트의 오른쪽 **`수정사업계획(시즌/복종)`** 블록 (INNER→TOP · SHOES→ACC 매핑). 왼쪽 블록은 25FW 실적이므로 쓰면 안 된다. 하위 카테고리 단위는 MDP 에 없어 '-' 로 비운다. MDP 갱신 시 NAS 의 새 파일을 `data/26FW_MDP.xlsb` 로 복사 |
