...
| 러닝 카드 지표 | 기초재고(시점재고 07/31) · 추가 기획량(신규품번 기획) · 입고량(신규품번) · 현재고(기초+입고) · 판매율 = (구품번 8/1 이후 판매 + 신규품번 누계판매) ÷ 현재고. 8/1 이후 판매는 8/1 이 포함된 주(W5, 07/27~)부터의 합. 러닝 카드 컬러 표는 신규품번에 기획된 컬러만 보여주고(기획·입고 = 신규품번, 누계 = 8/1 이후 구+신), 색상별 기초재고가 없어 색상별 판매율은 표시하지 않는다 |
| 스타일수·SKU·기획량 | **MDP 파일**(`data/26FW_MDP.xlsb`)의 **집계표 시트** 값을 그대로 쓴다 (*시즌 · *시즌/복종 블록, INNER→TOP · SHOES→ACC 매핑). MDP 시트를 품번으로 합치면 품번 미부여 스타일이 빠져 어긋난다. 하위 카테고리 단위는 MDP 에 없어 '-' 로 비운다. MDP 갱신 시 NAS 의 새 파일을 `data/26FW_MDP.xlsb` 로 복사 |
