...
| 스타일수·SKU·기획량 | **MDP 파일**(`data/26FW_MDP.xlsb`) MDP 시트를 스타일 블록(AI열 STY=1 시작) 단위로 읽어 **ED열(컨펌여부)='확정' 인 스타일만** 집계 (SKU = 컬러명 AM 행 수, 기획량 = 컬러별수량 AN 합, INNER→TOP · SHOES→ACC). 하위 카테고리 단위는 '-'. MDP 갱신 시 새 파일을 `data/26FW_MDP.xlsb` 로 복사 |
